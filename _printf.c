 ...
 * Return: numbber of chars printed
 */
int _printf(const char *format, ...)
{
	...
	for (j = 0; format && format[j] != '\0'; j++)
	{
		if (format[j] != '%')
		{
			...
			/* write(1,&format[i], 1) */
			/*This is the putchar alternative*/
			printed_chars++;
		}
		else
		{
 ...
 * @buff_ind: this is the index at which to add next char, represent the length
 */
